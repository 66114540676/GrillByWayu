...
describe('Orders API (e2e)', () => {
  ...
  // ทุก request แนบ token ที่ได้จากการ login ใน beforeEach
  const http = () => {
    const server = request(app.getHttpServer());
    const auth = `Bearer ${token}`;
    return {
      get: (url: string) => server.get(url).set('Authorization', auth),
      post: (url: string) => server.post(url).set('Authorization', auth),
      patch: (url: string) => server.patch(url).set('Authorization', auth),
      delete: (url: string) => server.delete(url).set('Authorization', auth),
    };
  };
  ...
  beforeEach(async () => {
    ...
    const login = await request(app.getHttpServer())
      .post('/auth/login')
      .send({
        username: process.env.AUTH_USERNAME || 'wayu',
  ...
  it('POST /orders/:id/clone สั่งซ้ำได้ id ใหม่', async () => {
    ...
    const res = await http().post(`/orders/${first.body.id}/clone`).expect(201);
  ...
  it('POST /orders/xxx/clone ไม่มีออเดอร์ → 404', async () => {
    await http().post('/orders/xxx/clone').expect(404);
  });
  ...
  it('POST /sets/:id/order ปรับน้ำจิ้ม แล้วเซ็ตต้นแบบไม่เปลี่ยน', async () => {
    ...
    const sets = await http().get('/sets').expect(200);
  ...
  it('GET /orders ใหม่สุดขึ้นก่อน', async () => {
    await http().post('/sets/standard/order').send({});
    await http().post('/sets/combo/order').send({});
  ...
  it('PATCH เพิ่มหมูในออเดอร์เนื้อวัวสุกกลาง → 400 และออเดอร์เดิมไม่เปลี่ยน', async () => {
    ...
    const orders = await http().get('/orders').expect(200);
    expect(orders.body).toEqual([created.body]);
  });

  it('PATCH /orders/xxx ไม่มีออเดอร์ → 404', async () => {
    await http().patch('/orders/xxx').send({ sauceId: 'suki-sauce' }).expect(404);
  });
  ...
  it('แก้ออเดอร์ต้นฉบับ ออเดอร์ที่สั่งซ้ำจากมันไม่เปลี่ยนตาม', async () => {
    const original = await http()
      .post('/orders')
      .send({ meatIds: ['beef-slice'], doneness: 'สุกกลาง', sauceId: 'jaew-sauce' });
    const copy = await http().post(`/orders/${original.body.id}/clone`).expect(201);
    await http()
  ...
  it('DELETE /orders/:id ลบแล้ว GET /orders ไม่มีออเดอร์นั้น', async () => {
    const keep = await http().post('/sets/standard/order').send({});
    const gone = await http().post('/sets/combo/order').send({});
    await http().delete(`/orders/${gone.body.id}`).expect(204);
  ...
  it('DELETE /orders/xxx ไม่มีออเดอร์ → 404', async () => {
    const res = await http().delete('/orders/xxx').expect(404);
  ...
  it('DELETE /orders ล้างทั้งหมด แต่เซ็ตต้นแบบยังอยู่ครบ', async () => {
    await http().post('/sets/standard/order').send({});
    await http().post('/sets/combo/order').send({});
    const res = await http().delete('/orders').expect(200);
  ...
  it('ลบออเดอร์ต้นฉบับ ออเดอร์ที่สั่งซ้ำยังอยู่ (clone เป็นคนละ object)', async () => {
    const original = await http()
      ...
      .send({ meatIds: ['beef-slice'], doneness: 'สุกกลาง', sauceId: 'jaew-sauce' });
    const copy = await http().post(`/orders/${original.body.id}/clone`).expect(201);
    await http().delete(`/orders/${original.body.id}`).expect(204);
